-- PITCHAT — schema inicial (Fase 1: fundação)
-- Rodar no SQL Editor do projeto Supabase DEDICADO ao PITCHAT (não o do PitBrain).
...
create policy webhook_events_by_social_account on webhook_events
  for all using (
    social_account_id is null or
    exists (select 1 from social_accounts sa where sa.id = social_account_id and is_workspace_member(sa.workspace_id))
  );

create policy jobs_by_workspace on jobs
